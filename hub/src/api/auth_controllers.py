...
def login():
    ...
    token_data = {'userid': user_jwt['preferred_username']}

    if (preferred_username == Settings.admin_user):
        token_data["role"]  = "admin"

    log.info(preferred_username)

    # TODO add the admin user to the authorization store
    ...
    # cookies are technically more secure than using local storage
